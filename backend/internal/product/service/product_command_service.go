...
func (s *productCommandService) CreateProduct(ctx context.Context, productDTO *productDto.ProductDTO) (*productDto.ProductDTO, error) {
	...
	if _, err := s.productRepo.FindBySKU(ctx, productDTO.SKU); err == nil {
		return nil, &errors.ErrorDto{Code: "DUPLICATE_SKU", Message: "SKU already exists"}
	}
	...
	for _, uomDTO := range productDTO.UOMs {
		...
		if err != nil {
			return nil, &errors.ErrorDto{Code: "INVALID_ID", Message: "Invalid UOM ID"}
		}
	...
	for _, catDTO := range productDTO.Categories {
		...
		if err != nil {
			return nil, &errors.ErrorDto{Code: "INVALID_ID", Message: "Invalid Category ID"}
		}
	...
	if err != nil {
		return nil, &errors.ErrorDto{Code: "INVALID_ID", Message: "Invalid Supplier ID"}
	}
	if _, err := s.supplierRepo.FindByID(ctx, supplierID); err != nil {
		return nil, &errors.ErrorDto{Code: "NOT_FOUND", Message: "Supplier not found"}
	}

	productDTO.ID = uuid.NewString()
	product := mapper.ToProduct(productDTO)

	// Create product in database
	if err := s.productRepo.Create(ctx, product); err != nil {
		return nil, &errors.ErrorDto{Code: "DATABASE_ERROR", Message: "Failed to create product"}
	}

	// Reload product to get all fields populated, including associations
	createdProduct, err := s.productRepo.FindByID(ctx, uuid.MustParse(productDTO.ID))
	if err != nil {
		return nil, &errors.ErrorDto{Code: "DATABASE_ERROR", Message: "Failed to retrieve created product"}
	}
...
func (s *productCommandService) UpdateProduct(ctx context.Context, id uuid.UUID, productDTO *productDto.ProductDTO) (*productDto.ProductDTO, error) {
	...
	if err := s.productRepo.Update(ctx, updatedProduct); err != nil {
		return nil, &errors.ErrorDto{Code: "DATABASE_ERROR", Message: "Failed to update product"}
	}

	// Reload product to get all fields populated, including associations
	reloadedProduct, err := s.productRepo.FindByID(ctx, id)
	if err != nil {
		return nil, &errors.ErrorDto{Code: "DATABASE_ERROR", Message: "Failed to retrieve updated product"}
	}
...
func (s *productCommandService) DeleteProduct(ctx context.Context, id uuid.UUID) error {
	...
	if err != nil {
		return &errors.ErrorDto{Code: "NOT_FOUND", Message: "Product not found"}
	}

	// Delete product
	err = s.productRepo.Delete(ctx, id)
	if err != nil {
		return &errors.ErrorDto{Code: "DATABASE_ERROR", Message: "Failed to delete product"}
	}
